...
public class JAbsence extends Model implements MongoModel {
    ...
    private static Query<JAbsence> queryToFindMeByUser(final ObjectId id) {
        return q().field("userId").equal(id);
    }
    ...
    public static JAbsence delete(final ObjectId id) {
    ...
    public static void deleteByUser(JUser user) {
        final Query<JAbsence> q = queryToFindMeByUser(user.id);
        final List<JAbsence> absences = q.asList();
        for(JAbsence absence: absences){
            DbFile.remove(absence.fileId);
        }
        JAbsenceDay.delete(user);

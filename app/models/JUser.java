...
public class JUser extends Model implements Subject {
    ...
    public static void unassignMission(final String username, final String missionId) {

        final JUser user = queryToFindMe(username).get();
        Iterables.removeIf(user.affectedMissions, new Predicate<JAffectedMission>() {
            @Override
            public boolean apply(@Nullable final JAffectedMission am) {
                return am.missionId.equals(ObjectId.massageToObjectId(missionId));
            }
        });
        save(user);
    }
